...
{
	static private TokenFilterFactory instance = null;
	...
	{
		if(singleton == 0) return new TokenFilterFactory();
		return instance;
	}
	...
	public TokenFilter getFilterByType(TokenFilterType type, TokenStream stream) 
	{
		...
		{
			case SYMBOL:
				return new SymbolsTokenFilter(stream);
			case NUMERIC:
				return new NumbersTokenFilter(stream);
			case CAPITALIZATION:
				return new CapsTokenFilter(stream);
			case STOPWORD:
				return new StopWordsTokenFilter(stream);
			case STEMMER:
				return new StemmersTokenFilter(stream);
			case ACCENT:
				return new AccentsTokenFilter(stream);
			case SPECIALCHARS:
